...
**Ссылка на установку и настройку от инженеров Р7**
...
**Ссылка на изменение доменных имен и обновление ssl-сертификатов модулей Корпоративного сервера 2024**
...
Открываем терминал меняем пароль на root и даём разрешение на подключение в файле ``/etc/ssh/sshd_config``<br> 
можно через редактор ``nano /etc/ssh/sshd_config``  параметр PermitRootLogin yes или через команды в консоле
```bash
$ sudo -i
...
```
Теперь можно заходить про root по ssh 
...
Добавляем из консоли, проверка существования и добавление командой
```bash
grep -q "net.ipv4.tcp_timestamps" /etc/sysctl.d/999-astra.conf || echo "net.ipv4.tcp_timestamps = 0" | sudo tee -a /etc/sysctl.d/999-astra.conf
grep -q "net.ipv6.conf.all.disable_ipv6" /etc/sysctl.d/999-astra.conf || echo "net.ipv6.conf.all.disable_ipv6 = 1" | sudo tee -a /etc/sysctl.d/999-astra.conf
grep -q "net.ipv6.conf.lo.disable_ipv6" /etc/sysctl.d/999-astra.conf || echo "net.ipv6.conf.lo.disable_ipv6 = 0" | sudo tee -a /etc/sysctl.d/999-astra.conf
```
```bash
...
sysctl -p
```
##### Если вы установили с графическим интерфейсом - Отключаем NetworkManager.

На серверах не должно быть динамического адреса.
```bash
...
Через команду из консоли
```bash
sudo sed -i '/iface eth0 inet static/,/^$/ s/address 10\.159\.15\.19/address 10.179.15.19/' /etc/network/interfaces
sudo sed -i '/iface eth0 inet static/,/^$/ s/gateway 10\.159\.15\.11/gateway 10.179.15.1/' /etc/network/interfaces
```
или так
...
address 10.179.15.18
netmask 255.255.255.0
gateway 10.179.15.1
```

Заменить на правильные данные вфайлеresov.conf
```bash
sudo tee /etc/resolv.conf > /dev/null << 'EOF'
search r16.rosreestr.ru
nameserver 10.179.15.1
nameserver 1.1.1.1
nameserver 10.128.140.2
EOF
```
**$${\color{#FF8800}REBOOT}$$**

После перезагрузки проверяем все ли настройки применились правильно
```bash
...
r16.rosreestr.ru
root@mx1:~#
root@mx1:~#ifquery eth0
address: 10.179.15.18
netmask: 255.255.255.0
gateway: 10.179.15.1
broadcast: 10.179.15.255

```

Открываем репозитории  ``nano /etc/apt/sources.list`` в редакторе или через команды в консоле

<img width="1278" height="210" alt="image" src="https://github.com/user-attachments/assets/f4b501e2-998f-46d2-9b2f-3dc0dac47e02" />

Или переписываем файл из консоли
```bash
...
#deb [trusted=yes] http://urrca-repo01.urrca.esk/repos/pt/ ptapp main
EOF
```

Теперь обновим пакеты и установим необходимые утилиты для дальнейшей работы
```bash
# apt update
...
# timedatectl set-timezone Europe/Moscow && date
```
...
```bash
127.0.0.1       localhost
10.179.15.18    gluster1 #ip адрес первого почтового сервера
10.179.15.18    gluster2 #ip адрес второго почтового сервера
```
>[!Warning]
...
```bash
127.0.0.1       localhost
10.179.15.18     mx10.r16.rosreestr.ru        mx10
10.179.15.19     mx20.r16.rosreestr.ru        mx20
...
##### Проделываем те же манипуляции, что и на сервере №1 как в п.2.1
...
>[!Warning]
>Инсструкция выполняется на всех нодах **$${\color{red}НА}$$ $${\color{red}СЕРВЕРЕ}$$ $${\color{red}№1}$$ и $${\color{red}НА}$$ $${\color{red}СЕРВЕРЕ}$$ $${\color{red}№2}$$**
...
>[!Warning]
>Это также выполняется на всех нодах **$${\color{red}НА}$$ $${\color{red}СЕРВЕРЕ}$$ $${\color{red}№1}$$ и $${\color{red}НА}$$ $${\color{red}СЕРВЕРЕ}$$ $${\color{red}№2}$$**
...
```
> - где: ip_srv1, ip_srv2 — адреса внутренней сети первого и второго почтового сервера<br>
> - у нас адреса в качестве примера эти 10.179.15.18,10.179.15.19 — адреса, которые слушает сервис; 5432 — порт, который сервис прослушивает.

Настройте аутентификацию пользователей в базе данных. Приведите к следующему виду файл ``/etc/postgresql/11/main/pg_hba.conf``<br> 
Для этого отредактируйте ``/etc/postgresql/11/main/pg_hba.conf``:
```cmd
...
```
> - вместо ``ip_srv1/32 и ip_srv2/32`` укажите IP-адрес двух серверов. Режим trust необходим для установки Корпоративного сервера<br>
> - у нас адреса в качестве примера эти 10.179.15.18,10.179.15.19
...
```
>[!Warning]
> - Далее инструкция ниже выполняется только **$${\color{red}НА}$$ $${\color{red}ПЕРВОМ}$$ $${\color{red}СЕРВЕРЕ}$$ $${\color{red}№1}$$**
...
```
>[!Warning]
> - Инструкция ниже выполняется только **$${\color{red}НА}$$ $${\color{red}ПЕРВОМ}$$ $${\color{red}СЕРВЕРЕ}$$ $${\color{red}№1}$$**
...
>[!Warning]
> - Инструкция ниже выполняется только **$${\color{red}НА}$$ $${\color{red}ВТОРОМ}$$ $${\color{red}СЕРВЕРЕ}$$ $${\color{red}№2}$$**
...
```
****
**$${\color{#a0ff00}УСТАНОВКА}$$**

#### 4. Установка Корпоративного сервера.

Выполнение установки по инструкции Установка Корпоративный сервер 2024 на ОС Астра Линукс<br> https://support.r7-office.ru/corporate-server2024/install-r7server/single/ustanovka-r7-disk/.

##### Предварительная подготовка
>[!Warning]
> - Установка выполняется сначала **$${\color{red}НА}$$ $${\color{red}ВТОРОМ}$$ $${\color{red}СЕРВЕРЕ}$$ $${\color{red}№2}$$**

Поместите скачанный дистрибутив в папку /mnt на сервер № 2.
...
```
>[!Warning]
> - Дистрибутивы CDinstall_*.zip и cddisk.zip в эту папку /mnt/
...
>[!Warning]
> - В папку ./distr положите ISO-образ установочного диска операционный системы. Файл должен быть с расширением 1.7.4-24.04.2023_14.23.iso.
> - Данный образ вы должны были скачать с официального сайта производителя<br>
> - Дистрибутив 1.7.4-24.04.2023_14.23.iso в эту папку /mnt/CDDiskPack/CDinstall_Astra_1.7.4/distr#<br>
...
root@mx1:~# ls /mnt/CDDiskPack/CDinstall_Astra_1.7.4/sslcert/
 rosreestr.ru.cer    r16.rosreestr.ru.cer    rosreestr.ru.key    r16.rosreestr.ru.key    'Корн Russian Trusted root.cer'    'Корн Russian Trusted sub.cer'
root@mx1:~#
```
...
openssl x509 -inform DER -in r16.rosreestr.ru.cer -out r16.rosreestr.ru.crt
```
...
```cmd
file rosreestr.ru.crt && file r16.rosreestr.ru.crt
должно значение
rosreestr.ru.crt: PEM certificate
r16.rosreestr.ru.crt: PEM certificate
```
...
cp r16.rosreestr.ru.crt /usr/local/share/ca-certificates/
...
**Укажите БД master и IP-адрес первого сервера 1 (мастер 10.179.15.18):**
...
**Необходимо указать имя сервера на котором вы проводите установку: mx20**

<img width="730" height="277" alt="image" src="https://github.com/user-attachments/assets/7aeae80c-e527-416d-b8b1-5be3ab6d5603" />

**Укажите IP-адрес сервера на котором вы проводите установку (данный сервер 10.179.15.19):**
...
После перезагрузки проверяем службы:
```cmd
sudo -i
# systemctl status supervisor.service && nginx -t && supervisorctl status && systemctl status rabbitmq-server.service
```
***Если ошибок нет, то будет такое сообщение*** 
```cmd
● supervisor.service - Supervisor process control system for UNIX
   Loaded: loaded (/lib/systemd/system/supervisor.service; enabled; vendor preset: enabled)
   Active: active (running) since Fri 2026-01-23 17:41:16 MSK; 1 day 20h ago
     Docs: http://supervisord.org
 Main PID: 940 (supervisord)
    Tasks: 538 (limit: 4915)
   Memory: 2.7G
      CPU: 2h 9min 36.595s
   CGroup: /system.slice/supervisor.service
           ├─ 940 /usr/bin/python3 /usr/bin/supervisord -n -c /etc/supervisor/supervisord.conf
           ├─2344 /usr/bin/dotnet /opt/r7-office/Api/R7.Storage.Server.Api.Host.dll
           ├─2346 /usr/bin/dotnet /opt/r7-office/FileStorage/FileStorage.Api.Host.dll
           ├─2347 /usr/bin/dotnet /opt/r7-office/Processing/R7.Storage.Server.Processing.Host.dll
           ├─2348 /usr/bin/dotnet /opt/r7-office/SearchApi/Bsa.Search.Api.Host.dll
           ├─2354 /usr/bin/dotnet /opt/r7-office/Service.Registry/Service.Registry.Host.dll
           └─2360 /usr/bin/dotnet /opt/r7-office/Sso.Api/R7.Sso.Api.Host.dll

янв 23 17:41:18 mx10.r16.rosreestr.ru supervisord[940]: 2026-01-23 17:41:18,046 INFO spawned: 'processing' with pid 2347
янв 23 17:41:18 mx10.r16.rosreestr.ru supervisord[940]: 2026-01-23 17:41:18,050 INFO spawned: 'searchapi' with pid 2348
янв 23 17:41:18 mx10.r16.rosreestr.ru supervisord[940]: 2026-01-23 17:41:18,054 INFO spawned: 'registry' with pid 2354
янв 23 17:41:18 mx10.r16.rosreestr.ru supervisord[940]: 2026-01-23 17:41:18,057 INFO spawned: 'apisso' with pid 2360
янв 23 17:41:19 mx10.r16.rosreestr.ru supervisord[940]: 2026-01-23 17:41:19,449 INFO success: api entered RUNNING state, process has stayed up for > than 1 seconds (startsecs)
янв 23 17:41:19 mx10.r16.rosreestr.ru supervisord[940]: 2026-01-23 17:41:19,450 INFO success: filestorage entered RUNNING state, process has stayed up for > than 1 seconds (startsecs)
янв 23 17:41:19 mx10.r16.rosreestr.ru supervisord[940]: 2026-01-23 17:41:19,450 INFO success: processing entered RUNNING state, process has stayed up for > than 1 seconds (startsecs)
янв 23 17:41:19 mx10.r16.rosreestr.ru supervisord[940]: 2026-01-23 17:41:19,450 INFO success: searchapi entered RUNNING state, process has stayed up for > than 1 seconds (startsecs)
янв 23 17:41:19 mx10.r16.rosreestr.ru supervisord[940]: 2026-01-23 17:41:19,450 INFO success: registry entered RUNNING state, process has stayed up for > than 1 seconds (startsecs)
янв 23 17:41:19 mx10.r16.rosreestr.ru supervisord[940]: 2026-01-23 17:41:19,450 INFO success: apisso entered RUNNING state, process has stayed up for > than 1 seconds (startsecs)
2026/01/25 13:49:42 [info] 27235#27235: Using 116KiB of shared memory for nchan in /etc/nginx/nginx.conf:63
2026/01/25 13:49:42 [info] 27235#27235: Using 131072KiB of shared memory for nchan in /etc/nginx/nginx.conf:63
nginx: the configuration file /etc/nginx/nginx.conf syntax is ok
nginx: configuration file /etc/nginx/nginx.conf test is successful
cddisk:api                       RUNNING   pid 2344, uptime 1 day, 20:08:24
cddisk:apisso                    RUNNING   pid 2360, uptime 1 day, 20:08:24
cddisk:filestorage               RUNNING   pid 2346, uptime 1 day, 20:08:24
cddisk:processing                RUNNING   pid 2347, uptime 1 day, 20:08:24
cddisk:registry                  RUNNING   pid 2354, uptime 1 day, 20:08:24
cddisk:searchapi                 RUNNING   pid 2348, uptime 1 day, 20:08:24
● rabbitmq-server.service - RabbitMQ Messaging Server
   Loaded: loaded (/lib/systemd/system/rabbitmq-server.service; enabled; vendor preset: enabled)
   Active: active (running) since Fri 2026-01-23 17:41:21 MSK; 1 day 20h ago
 Main PID: 976 (beam.smp)
   Status: "Initialized"
    Tasks: 595 (limit: 4915)
   Memory: 169.5M
      CPU: 59min 41.661s
   CGroup: /system.slice/rabbitmq-server.service
           ├─ 961 /bin/sh /usr/sbin/rabbitmq-server
           ├─ 976 /usr/lib/erlang/erts-10.4.4/bin/beam.smp -W w -A 512 -MBas ageffcbf -MHas ageffcbf -MBlmbcs 512 -MHlmbcs 512 -MMmcs 30 -P 1048576 -t 5000000 -stbt db -zdbbl 128000 -K true -- -root /usr/lib/erlang -progname erl -- -home /var/lib/rabbitmq -- -pa /usr/lib/rabbitm
           ├─2065 erl_child_setup 65536
           ├─3877 inet_gethost 4
           └─3885 inet_gethost 4

янв 23 17:41:16 mx10.r16.rosreestr.ru systemd[1]: Starting RabbitMQ Messaging Server...
янв 23 17:41:21 mx10.r16.rosreestr.ru systemd[1]: rabbitmq-server.service: Supervising process 976 which is not our child. We'll most likely not notice when it exits.
янв 23 17:41:21 mx10.r16.rosreestr.ru systemd[1]: Started RabbitMQ Messaging Server.

```

**$${\color{#a0ff00}УСТАНОВКА}$$$${\color{#a0ff00}НА}$$ $${\color{#a0ff00}ВТОРОМ}$$$${\color{#a0ff00}СЕРВЕРЕ}$$$${\color{#a0ff00}№2}$$$${\color{#a0ff00}ЗАВЕРШЕНА}$$**
...
##### Предварительная подготовка
>[!Warning]
> - Установка продолжаем выполняеть **$${\color{red}НА}$$ $${\color{red}ПЕРВОМ}$$ $${\color{red}СЕРВЕРЕ}$$ $${\color{red}№1}$$**

Перейдите и скачайте дистрибутив на сервер №1.
...
**Необходимо указать имя сервера на котором вы проводите установку: mx10**

<img width="730" height="277" alt="image" src="https://github.com/user-attachments/assets/d918098a-14bd-409a-b480-4eb3da579db0" />

**Укажите IP-адрес сервера на котором вы проводите установку  (данный сервер 10.179.15.18):**
...
"dbHost": "localhost"
```
Где потребуется заменить localhost на адрес текущего мастера (первого сервера), из примера 10.179.15.18
...
Пример:  
`_submission._tcp.r16.rosreestr.ru. 3600 IN SRV 10 5 587 mx1.r16.rosreestr.ru.`
...
_submission._tcp.r16.rosreestr.ru.  3600 IN SRV 10 5 587 r16.rosreestr.ru.

; Получение почты (IMAP)
_imaps._tcp.r16.rosreestr.ru.       3600 IN SRV 10 0 993 r16.rosreestr.ru.

; Autodiscover (для Outlook/Thunderbird)
_autodiscover._tcp.r16.rosreestr.ru. 3600 IN SRV 10 0 443 r16.rosreestr.ru.
```
...
3. Заполните поля по шаблону:  
   ...
   - **Target:** `r16.rosreestr.ru` (ваш сервер)  
   - **Priority:** `10`  
   - **Weight:** `5`  
   - **Port:** `587`  
...
Используйте команды:  
```bash
dig SRV _submission._tcp.r16.rosreestr.ru
# или
nslookup -type=SRV _submission._tcp.r16.rosreestr.ru
```
...
|--------|-----------------------|---------------|--------------|
|MX	 |mx10.r16.rosreestr.ru.  |300            |10		|
|TXT     |r16.rosreestr.ru.        |TTL            |		|
|	 |v=spf1+mx~all          |300            |		|
|A       |mx10.r16.rosreestr.ru.  |TTL            |		|
|	 |10.179.15.18          |300            |		|
   
Теперь по подробней.

**MX-запись** — это тип DNS-записи, который указывает, какой сервер отвечает за прием электронной почты для домена.<br> 
MX-запись указывает на доменное имя почтового сервера (например, в нашем случае - mx10.r16.rosreestr.ru), которое должно иметь A-запись (IPv4), чтобы связать домен с IP-адресом. Чем меньше число, тем выше приоритет.
...
```bash
nslookup -type=mx r16.rosreestr.ru
или так
dig mx r16.rosreestr.ru
```
...
SPF-запись v=spf1 +mx ~all разрешит отправку писем только с mx10.r16.rosreestr.ru, а письма с других серверов будут отклонены или помечены.
...
autodiscover.IN.A 10.179.15.18
...
Создайте папку `mkdir /var/www/autodiscover`<br>
Создайте файл `touch /var/www/autodiscover/autodiscover.xml`<br>
...
<img width="1298" height="381" alt="image" src="https://github.com/user-attachments/assets/cf18f779-0ae7-4f47-b6bf-ddcefb16e432" />
...
server {
    listen 443 ssl;
    server_name autodiscover.r16.rosreestr.ru;
...
<img width="1287" height="425" alt="image" src="https://github.com/user-attachments/assets/7b71c0e1-72ba-4ad6-938f-5906f3cdb40f" />
...
<img width="1128" height="99" alt="image" src="https://github.com/user-attachments/assets/451644bd-8bf6-4a7d-a99c-68f891f7ddf7" />
...
<img width="1288" height="326" alt="image" src="https://github.com/user-attachments/assets/f6617731-0c92-4e72-aa9a-14e51c8aff22" />
...
<img width="1336" height="655" alt="image" src="https://github.com/user-attachments/assets/b493b78a-2165-46ca-9cd9-364c568e5369" />
...
4. **Тест через curl**:
```bash
   curl https://autodiscover.r16.rosreestr.ru/autodiscover/autodiscover.xml
   curl autodiscover.r16.rosreestr.ru:443
```

<img width="1307" height="510" alt="image" src="https://github.com/user-attachments/assets/6b6ac90d-40a0-4811-9959-bd7c3c2d062f" />
...
| *SSL-ошибки* | Убедитесь, что сертификат включает `autodiscover.r16.rosreestr.ru` |
...
| Блокировка фаерволом              | Откройте порт 443 для `autodiscover.r16.rosreestr.ru`.        |
...
1. **Интеграция с LDAP/AD**:  
   ...
   ```xml
   <LoginName>%USERNAME%@r16.rosreestr.ru</LoginName>
   ```
...
```bash
firewall-cmd --zone=internal --add-source=10.179.15.0/24
```
